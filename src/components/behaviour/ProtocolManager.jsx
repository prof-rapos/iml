...
function SignalRow({ protocolId, sig, update, remove, readOnly }) {
  return (
    <div style={{ display: 'flex', gap: 6, alignItems: 'center', marginBottom: 4 }}>
      <input
        style={{ ...input, flex: 1, opacity: readOnly ? 0.6 : 1 }}
        value={sig.name} disabled={readOnly}
        onChange={(e) => update(protocolId, sig.id, { name: e.target.value })}
      />
      <select
        style={{ ...select, width: 66, opacity: readOnly ? 0.6 : 1 }}
        value={sig.direction} disabled={readOnly}
        onChange={(e) => update(protocolId, sig.id, { direction: e.target.value })}
      >
        <option value="in">in</option>
        <option value="out">out</option>
      </select>
      {!readOnly && (
        <button onClick={() => remove(protocolId, sig.id)}
...
function ProtocolCard({ proto, actions }) {
  ...
  return (
    <div style={{ background: CARD_BG, border: `1px solid ${BORDER}`, borderRadius: 8, padding: 12, marginBottom: 10 }}>
      ...
      {proto.signals.map((sig) => (
        <SignalRow key={sig.id} protocolId={proto.id} sig={sig} update={actions.updateSignal} remove={actions.deleteSignal} readOnly={readOnly} />
      ))}
      {!readOnly && (
...
export default function ProtocolManager({ onClose }) {
  ...
  const actions = { updateProtocol, deleteProtocol, addSignal, updateSignal, deleteSignal };
    ...
    >
      ...
      >
        ...
        <div style={{ padding: '14px 20px', overflowY: 'auto', flex: 1 }}>
          ...
          {SYSTEM_PROTOCOLS.map((p) => <ProtocolCard key={p.id} proto={p} actions={actions} />)}
          {protocols.map((p) => <ProtocolCard key={p.id} proto={p} actions={actions} />)}
        </div>
